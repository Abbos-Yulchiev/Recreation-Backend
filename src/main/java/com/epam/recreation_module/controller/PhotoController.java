...
/**
 * Class {@code CommentaryController} is an endpoint of the API which used for Photo
 ...
 * So that {@code CommentaryController} is accessed by sending requests to /photo
 *
...
public class PhotoController {
     ...
     * Annotated by ApiOperation with parameters: value = "Get a photo by recreation ID"
     ...
     * Annotated by ApiOperation with parameters: value = "Add a photo to recreation by recreation ID"
     * Annotated by {@link GetMapping} with parameter value = /get/{recreationId}
     *
     * @param recreationId - Recreation ID which wanted to see photos
     * @param response return the photo's content
    ...
    public ResponseEntity<?> addPhoto(@ApiParam(name = "Recreation id", type = "Long", value = "ID of photo", example = "1") @PathVariable Long recreationId,
                                      @ApiParam(type = "file") @RequestParam(name = "file") MultipartFile[] files) {
        ApiResponse response = photoService.addPhoto(recreationId, files);
    ...
    public ResponseEntity<?> deletePhoto(@ApiParam(name = "Photo id", type = "Long", value = "ID of photo", example = "1") @PathVariable Long photoId) {
