...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderServiceImpl(OrderRepository orderRepository, TicketRepository ticketRepository,
                            EventRepository eventRepository, ExternalConnections externalConnections,
    ...
    public ResponseEntity<?> getMyTickets() {
        try {
            ...
            List<Object> orderByUser = orderRepository.findOrderedTicketsByUser(user.getId());
    ...
    public ResponseEntity<?> getMyRecreations() {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        List<Object> orderByUser = orderRepository.findOrderedRecreationsByUser(user.getId());
    ...
    public ResponseEntity<?> getOrdersByRecreationId(Long id) {
        ...
        List<Object> orderList = orderRepository.findAllByOrderByRecreationId(id);
        return ResponseEntity.status(HttpStatus.OK).body(new ApiResponse("Orders", true, orderList));
    }
        ...
        try {
            ...
            for (Long ticketId : orderTicketDTO.getTicketsId()) {
                Ticket ticket = ticketRepository.findById(ticketId)
                        .orElseThrow(() -> new NotFoundException("Invalid ticket"));
                Event event = eventRepository.findById(ticket.getEventId())
                        .orElseThrow(() -> new NotFoundException("Invalid Event Id"));

                LocalDateTime time = LocalDateTime.now();
                boolean before = event.getEndTime().isBefore(time);
                if (before) return ResponseEntity.status(HttpStatus.FOUND).body("Event Already finished");
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Error occurred. Not found!");
        }
    }
    ...
    public ResponseEntity<?> orderRecreation(OrderRecreationDTO orderRecreationDTO) {
        ...
        Recreation recreation = recreationRepository.findById(orderRecreationDTO.getRecreationId())
                .orElseThrow(() -> new NotFoundException("Invalid Recreation ID!"));

        LocalDateTime closingTime = recreation.getClosingTime();
        ...
        LocalDateTime leavingTime = visitingTime;

        int minute = visitingTime.getMinute();
        double min = orderRecreationDTO.getVisitingDuration() / 0.5;
        if (minute >= 30 && min % 2 != 0) {
            leavingTime = visitingTime.plusHours((int) orderRecreationDTO.getVisitingDuration() + 1);
            leavingTime = visitingTime.minusMinutes(30);
        } else {
            leavingTime = visitingTime.plusHours((long) orderRecreationDTO.getVisitingDuration());
            leavingTime = visitingTime.plusMinutes(minute);
        }


        double price = recreation.getPrice() *
                orderRecreationDTO.getVisitingDuration()
                * orderRecreationDTO.getVisitorsNumber();
    ...
    public ResponseEntity<?> paymentConfirmation(PaymentConfirmationDTO paymentConfirmationDTO) {
        ...
        Order order = orderRepository.getUserLastOrder(paymentConfirmationDTO.getCitizenCardId())
                .orElseThrow(() -> new NotFoundException("Invalid Order Id"));
