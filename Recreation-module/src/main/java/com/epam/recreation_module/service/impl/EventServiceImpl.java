...
public class EventServiceImpl implements EventService {
    ...
    @Override
    public ResponseEntity<?> editEventStatus(Long eventId, String status) {
    ...
    public ResponseEntity<?> confirmEvent(ConfirmEventDTO body) {
        ...
        Event event = eventRepository.findById(body.getEventId())
                .orElseThrow(() -> new NotFoundException("Event not found with Id:" + body.getEventId()));
    ...
    private ResponseEntity<?> eventSetter(EventDTO eventDTO, List<Recreation> eventList, Event event) {
        ...
        List<String> startTimes = eventDTO.getStartTime();
        String startTime = startTimes.get(0);
        List<String> endTimes = eventDTO.getEndTime();
        ...
        return ResponseEntity.status(response.getStatusCode())
                .body(new ApiResponse("The event created. Event Id: " + save.getId(), true));
    }
}
